...
class MainActivity : Activity() {
    ...
    private fun buildUi() {
        ...
        text("FORGE", 28, true)
        text("ANDROID APP BUILDER • LOCAL OPEN-SOURCE AI", 11, true)
        text("Qwen3 1.7B is the default local coding assistant. No Gemini key required.", 14, false)
        ...
        section("LOCAL AI • QWEN3 1.7B")
        text("Run Qwen3-1.7B-GGUF locally with llama.cpp or another OpenAI-compatible server.", 13, false)
        endpoint = edit("OpenAI-compatible endpoint", 1)
        model = edit("Model", 1)
        key = edit("Optional local API key", 1)
        endpoint.setText(prefs.getString("endpoint", "http://127.0.0.1:8080/v1"))
        model.setText(prefs.getString("model", "Qwen3-1.7B-GGUF:Q8_0"))
        key.setText(prefs.getString("key", ""))
        button("SAVE AI SETTINGS") { saveSettings(); toast("Qwen3 settings saved") }
        button("TEST QWEN3") { runAi("Reply with exactly: FORGE QWEN3 OK") }
        ...
        button("GENERATE DATA SHEET") {
            runAi("Create a technical datasheet for: ${sheet.text}. Include identity, specifications, pinout/connectors when verified, operating limits when sourced, compatibility, test procedure, safety notes and source fields. Never invent values. Mark unavailable values UNKNOWN.")
        }
        button("IMPORT SOURCE TEXT") {
        ...
        section("RESULT")
        output = textView(13, false)
        output.text = "Ready. Start your local Qwen3 server, then press TEST QWEN3."
    }
    ...
    private fun runAiBuild(description: String) {
        ...
        runAi("""
            Build a COMPLETE Android application for this requirement: ${description.trim()}
            Return ONLY complete MainActivity.kt source. Kotlin JVM 17, compileSdk 35, minSdk 24.
            No pseudocode, TODOs, fake handlers, placeholder buttons, invented APIs or omitted logic.
            Every visible control must have a real handler. Include validation, persistence and error handling where required.
        """.trimIndent()) { code ->
            try {
                lastProject = ProjectBuilder.buildProject(
                    "ForgeGenerated",
                    "com.forge.generated",
                    code.removeCodeFences()
                )
                output.text = "PROJECT GENERATED AND PACKAGED\n\n${lastProject!!.size} bytes\n\nExport the ZIP and build it with Gradle/Android Studio."
            } catch (e: Exception) {
                output.text = "PROJECT PACKAGING FAILED\n\n${e.message}"
            }
        }
    }
    ...
    private fun runAi(prompt: String, onSuccess: (String) -> Unit = { output.text = it }) {
        ...
        saveSettings()
        output.text = "CONTACTING LOCAL QWEN3..."
        io.execute {
            try {
                val c = AiClient(
                    prefs.getString("endpoint", "http://127.0.0.1:8080/v1")!!,
                    prefs.getString("model", "Qwen3-1.7B-GGUF:Q8_0")!!,
                    prefs.getString("key", "")!!
                )
                val result = c.ask(prompt)
                runOnUiThread { onSuccess(result) }
            } catch (e: Exception) {
                runOnUiThread { output.text = "LOCAL QWEN3 REQUEST FAILED\n\n${e.message}" }
            }
        }
    }
    ...
    private fun exportProject() {
        ...
        startActivityForResult(Intent(Intent.ACTION_CREATE_DOCUMENT).apply {
            ...
            putExtra(Intent.EXTRA_TITLE, "Forge-Android-Project.zip")
        }, 42)
    }
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if (requestCode == 42 && resultCode == RESULT_OK && data?.data != null) try {
            contentResolver.openOutputStream(data.data!!)?.use { it.write(lastProject!!) }
    ...
    private fun textView(size: Int, bold: Boolean) = TextView(this).apply {
    ...
    private fun edit(hint: String, lines: Int) = EditText(this).apply {
        this.hint = hint
        setHintTextColor(0xFF777B85.toInt())
        setTextColor(0xFFE8EAF0.toInt())
        setBackgroundColor(0xFF17191E.toInt())
        setPadding(d(12), d(10), d(12), d(10))
        minLines = lines
        gravity = Gravity.TOP
        root.addView(this, LinearLayout.LayoutParams(-1, if (lines > 1) d(120) else d(56)))
    }
    private fun button(label: String, action: () -> Unit) = Button(this).apply {
        text = label
        setOnClickListener { action() }
        root.addView(this, LinearLayout.LayoutParams(-1, d(54)))
    }
